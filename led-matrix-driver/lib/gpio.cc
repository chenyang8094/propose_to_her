...
{
/*static*/ const uint32_t GPIO::kValidBits = ((1 << 0) | (1 << 1) | // RPi 1 - Revision 1 accessible
...
static bool DetermineIsRaspberryPi2()
{
  ...
  uint64_t mem_size = 0;
  if ((mem_size_key = strstr(buffer, "mem_size=")) != NULL && (sscanf(mem_size_key + strlen("mem_size="), "%" PRIx64, &mem_size) == 1) && (mem_size >= 0x3F000000))
  {
    return true;
  }
  return false;
...
static uint32_t JitterAllowanceMicroseconds()
{
  ...
  static int allowance_us = EMPIRICAL_NANOSLEEP_OVERHEAD_US + (IsRaspberryPi2() ? EMPIRICAL_NANOSLEEP_EXTRA_OVERHEAD_US : 0);
  return allowance_us;
}
